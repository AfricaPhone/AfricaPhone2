...
describe('ContestApplicationClient', () => {
  it('blocks submission when the contest is closed', () => {
    ...
    expect(screen.getByText('La phase de candidatures est clôturée.')).toBeInTheDocument();
  ...
  it('exposes validation errors when required fields are missing', async () => {
    ...
    fireEvent.change(screen.getByLabelText(/Média ou organe/i), { target: { value: 'Radio Test' } });
    fireEvent.change(screen.getByLabelText(/Biographie courte/i), { target: { value: 'Bio courte' } });
  ...
  it('saves and restores draft data from localStorage', async () => {
    ...
    fireEvent.change(screen.getByLabelText(/Média ou organe/i), { target: { value: 'TV Bénin' } });
    fireEvent.change(screen.getByLabelText(/Biographie courte/i), { target: { value: 'Reporter terrain.' } });
