...
const mapContest = (snapshot: DocumentSnapshot<DocumentData>): Contest | null => {
  ...
  const voteOpensAtValue = data.voteOpensAt ?? data.vote_opens_at ?? data.vote_opensAt;
...
export const fetchActiveContestId = async (): Promise<string | null> => {
  try {
    ...
    console.warn('contestService.fetchActiveContestId: unable to fetch contest', error);
    return null;
  }
};
