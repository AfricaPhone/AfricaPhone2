...
def format_cutoff(snapshot: Dict[str, Any]) -> Dict[str, str]:
  ...
  return {
    "local": f"{local_cutoff.strftime('%d/%m/%Y %H:%M:%S')} ({local_label})",
...
def build_table(snapshot: Dict[str, Any], total_votes: int) -> Table:
  ...
  header = ["#", "Candidat", "Votes ", "% des votes"]
  data = [header]
...
def render_pdf(snapshot_path: Path, output_path: Path) -> None:
  ...
  story = [
    Paragraph("CLASSEMENT OFFICIEL", title_style),
    Paragraph("Concours : Votes des Artistes", subtitle_style),
    Paragraph(f"Arrêté au : {cutoff_str['local']}", subtitle_style),
    Paragraph(f"Total des votes comptabilisés : {format_int(total_votes)}", subtitle_style),
    Spacer(1, 12),
  ]
...
def main(argv: list[str]) -> None:
  if len(argv) < 2:
    print("Usage: python scripts/render-ranking-pdf.py input.json [output.pdf]")
    sys.exit(1)
