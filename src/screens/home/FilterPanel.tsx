// src/screens/home/FilterPanel.tsx
...
const FilterPanel: React.FC<Props> = ({ onClose, onApply }) => {
  const { brands } = useProducts();
  const navigation = useNavigation<any>();

  const { filters, setPriceRange, toggleBrand, setCapacity, setPromotion, setVedette, resetFilters } = useFilters();
  ...
  const handleApply = () => {
    ...
    // On ferme le panneau après application pour voir tous les résultats
    onClose();
  };
    ...
    <SafeAreaView style={styles.container} edges={['bottom']}>
      ...
      <View style={styles.footer}>
        <TouchableOpacity style={[styles.footerButton, styles.applyButton]} onPress={handleApply}>
          <Text style={[styles.footerButtonText, styles.applyButtonText]}>VOIR LES RÉSULTATS</Text>
        </TouchableOpacity>
      </View>
    </SafeAreaView>
  );
...
const styles = StyleSheet.create({
  ...
  resetLink: { color: '#FF7A00', fontWeight: '600' },
